...
export function validateLogin(
  ...
  const passwordValidation: [boolean, string] =
...
export const validateRegister = (
  ...
  const passwordValidation1: [boolean, string] =
    validatePasswordFormat(password);
  if (passwordValidation1[0] === false) {
    return passwordValidation1;
  }

  const passwordValidation2: [boolean, string] = validatePasswordMatch(
    password,
    confirmPassword
  );
...
export const validateEmailVerification = (
  ...
  if (userID.length < 1) {
    return [
      false,
      "Incorrect userID length, check your email and copy paste !",
    ];
  } else if (verificationCode.length < 1) {
    return [
      false,
      "Incorrect email verification code length, check your email and copy paste !",
    ];
  } else {
...
export const validateForgotPasswordVerify = (
  verificationCode: string
): [boolean, string] => {
  return verificationCode.length === FORGOT_PASSWORD_VERIFICATION_CODE_LENGTH
    ? [true, "Correct length"]
...
const validatePasswordFormat = (inputText: string): [boolean, string] => {
  const lengthValidation: [boolean, string] = lengthChecker(
    inputText,
    MIN_PASSWORD_LENGTH,
